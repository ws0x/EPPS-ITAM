...
import { decideRequestAction, type RequestActionState } from "@/lib/actions/requests";
      ...
      <Card className="w-full max-w-lg border-teal-500/20 bg-teal-500/[0.01]">
        <CardContent className="pt-8 flex flex-col items-center text-center gap-4">
          ...
          <div className="flex flex-col gap-1">
            <h2 className="text-xl font-bold">Request {decision === "approved" ? "Approved" : "Rejected"}</h2>
            <p className="text-sm text-muted-foreground mt-2 max-w-sm">
              Your decision has been logged and the IT department has been notified. You can now close this window.
            </p>
          </div>
        </CardContent>
      </Card>
  ...
  return (
    <Card className="w-full max-w-lg border-primary/20">
      <CardHeader className="bg-primary/[0.01] border-b pb-4">
        <CardTitle className="text-lg text-primary">Review Approval Request</CardTitle>
        <CardDescription>Verify request details before choosing action.</CardDescription>
      </CardHeader>
      <CardContent className="pt-6 flex flex-col gap-6">
        {/* Detail List */}
        <div className="grid grid-cols-3 gap-y-3 text-sm">
          <span className="text-muted-foreground font-semibold">Requester</span>
          <span className="col-span-2 font-medium">{requesterName}</span>

          <span className="text-muted-foreground font-semibold">Item</span>
          <span className="col-span-2 font-medium">{itemName}</span>

          <span className="text-muted-foreground font-semibold">Quantity</span>
          <span className="col-span-2 font-mono">{quantity}</span>
        ...
        <div className="flex flex-col gap-4 border-t pt-4">
          <div className="flex flex-col gap-2">
            ...
            <Textarea
              id="rejectionReason"
              placeholder="Provide a brief explanation if you plan to reject this request..."
          ...
          <div className="flex gap-3 justify-end mt-2">
            ...
            >
              {pending && decision === "approved" ? "Approving..." : "Approve Request"}
            </Button>
          </div>
        </div>
      </CardContent>
